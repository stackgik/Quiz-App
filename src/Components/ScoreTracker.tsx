import { useQuiz } from '../contexts/QuizContext';

function ScoreTracker() {
  const { rightAnswer, wrongAnswer } = useQuiz();
  const wrong = (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="20"
      height="20"
      viewBox="0 0 24 24"
    >
      <g
        fill="none"
        stroke="currentColor"
        stroke-linecap="round"
        stroke-linejoin="round"
        stroke-width="2"
      >
        <circle cx="12" cy="11.999" r="9" />
        <path d="m15 9l-6 6m0-6l6 6" />
      </g>
    </svg>
  );

  const right = (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="20"
      height="20"
      viewBox="0 0 24 24"
    >
      <path
        fill="currentColor"
        d="m10.6 13.8l-2.15-2.15q-.275-.275-.7-.275t-.7.275q-.275.275-.275.7t.275.7L9.9 15.9q.3.3.7.3t.7-.3l5.65-5.65q.275-.275.275-.7t-.275-.7q-.275-.275-.7-.275t-.7.275zM12 22q-2.075 0-3.9-.788t-3.175-2.137q-1.35-1.35-2.137-3.175T2 12q0-2.075.788-3.9t2.137-3.175q1.35-1.35 3.175-2.137T12 2q2.075 0 3.9.788t3.175 2.137q1.35 1.35 2.138 3.175T22 12q0 2.075-.788 3.9t-2.137 3.175q-1.35 1.35-3.175 2.138T12 22m0-2q3.35 0 5.675-2.325T20 12q0-3.35-2.325-5.675T12 4Q8.65 4 6.325 6.325T4 12q0 3.35 2.325 5.675T12 20m0-8"
      />
    </svg>
  );

  return (
    <div className="score-tracker">
      <div className="wrongs marks">
        <span className="wrong-icon">{wrong}</span>
        <span className="wrong-num">{wrongAnswer}</span>
      </div>
      <div className="rights marks">
        <span className="right-icon">{right}</span>
        <span className="right-num">{rightAnswer}</span>
      </div>
    </div>
  );
}

export default ScoreTracker;
